...
Une attention particulière est portée à la modification du nombre de bits des échantillons, de
la fréquence d'échantillonnage utilisée ainsi qu'au contenu fréquentiel sauvegardé, afin d'analyser
...
acquis par le processus suivant. Un dictionnaire de plusieurs impacts espacés de 5mm chacun sur une ligne droite
est enregistré par le capteur piézoélectrique. Le signal de ces impacts est enregistré sous la forme d'un vecteur.
...
mi-hauteur. Un paramètre constant est aussi ajouté à la gaussienne afin d'ajuster son "plancher" en hauteur.
...
Ainsi, pour chaque test, le dictionnaire est modifié, puis ce processus de calcul est appliqué. Les calculs
...
$F$ 
pour revenir à son échelle d'origine. Avec cette méthode, plusieurs réductions du nombre de bits ont été effectuées
sur le dictionnaire original à 32 bits, afin d'essayer de voir si les résultats restent concluants malgré cette
détérioration. Les résultats suivants ont été obtenus et sont présentés dans le tableau \ref{nb_bit_tab}.
\begin{table}[H]
    \centering
    \begin{tabular}{l c c}
    \hline
    Dictionnaire & Résolution (cm) & Contraste  \\ \hline
    1 bits & $0 \pm 800$ & $0 \pm 100$ \\
    2 bits & $0 \pm 20$ & $0 \pm 5$ \\
    3 bits & $4 \pm 6$ & $0 \pm 2$ \\
    4 bits & $4 \pm 3$ & $0 \pm 1$ \\
    6 bits & $4.2 \pm 0.6$ & $0.6 \pm 0.3$ \\
    8 bits & $4.2 \pm 0.2$ & $0.58 \pm 0.09$ \\
    16 bits & $5.0 \pm 0.2$ & $0.70 \pm 0.08$ \\
    32 bits & $6.39 \pm 0.08$ & $0.62 \pm 0.02$ \\
    \hline
    \end{tabular}
...
26 valeurs de fréquences ont été testées, mais seules les valeurs non aberrantes sont
...
façon, un moins grand nombre de bits est nécessaire pour conserver l'information.
...
Heaviside, qui retourne à zéro comme valeur jusqu'au paramètre et 1 pour des valeurs 
...
fluctuent beaucoup plus, avec des résolutions qui dépassent 10 cm, des incertitudes 
...
qui sont plus du double des valeurs attendues et des incertitudes très grandes par 
...
En se rappelant l'objectif de ces tests, soit de voir à réduire au maximum la 
...
\begin{figure}[H]
    \centering
    \includegraphics[scale=0.25]{Screenshot 2024-10-20 230411.png}
    \caption{Preuve de correction par Antidote.}
    \label{antidote}
\end{figure}
