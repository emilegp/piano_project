...
dx = 2e-2;            % grid points spacing in the x direction [m]
dy = 2e-2;            % grid points spacing in the y direction [m]
...
%% Define sensors only on the material (excluding air)
clear sensor

% Liste des positions des capteurs qui sont sur le matériau (où la vitesse du son n'est pas celle de l'air)
sensor_positions = [];
for i = 1:Nx
    for j = 1:Ny
...
% Nombre de pas de temps et intervalle de temps
Nt = size(sensor_data_reversed_full, 3);  % Nombre de pas de temps à partir des données
dt = 5e-7;  % Utiliser le même pas de temps que dans la simulation (en secondes)

% Créer le tableau de temps en millisecondes
t_array = (0:Nt-1) * dt * 1e6;  % Conversion en millisecondes

% Sélectionner un capteur basé sur sa position dans le masque
n = 400;  % Vous pouvez ajuster cette valeur pour choisir un capteur

% Récupérer les positions des capteurs (masque des capteurs)
sensorXGrid = sensor_positions(:, 1);  % Coordonnées X des capteurs
sensorYGrid = sensor_positions(:, 2);  % Coordonnées Y des capteurs

% Récupérer la position du capteur n
i = sensorXGrid(n);  % Position X du capteur dans la grille
j = sensorYGrid(n);  % Position Y du capteur dans la grille

% Extraire les données temporelles pour le capteur spécifié
sensor_signal = squeeze(sensor_data_reversed_full(i, j, :));  % Extraction du signal pour le capteur (i, j)

% Tracer l'amplitude en fonction du temps (en millisecondes)
figure;
plot(t_array, sensor_signal);
xlabel('Temps[µs]');  % Échelle de temps en millisecondes
ylabel('Amplitude');
title(['Évolution du signal pour le capteur à la position (', num2str(i), ', ', num2str(j), ')']);
grid on;
