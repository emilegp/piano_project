...
l'identité du produit de convultion. On a donc
%
...
% sont intervertis. 
Pour chaque source, le signal reçu \textit{$S_ij$} est devenu:
\[S_{ij}(t)=h_{P_jS_i}(-t).\]
Une fois la réponse impulsionnelle obtenue pour tout les emplacements désirés 
sur la plaque, les décalages entre les signaux ont été retirés en gardant seulement 
la fenêtre de la réponse impulsionnelle.
...
Pour chaque point de la bande, la corrélation entre le signal de point et celui 
...
pour les graphiques; la moyenne et l'écart-type pour chaque variable sont calculées.




%Étapes (AU PASSÉÉÉ!!)
%1- Créer les configurations (Matériau + Géométrie + Position du capteur)
...
%1.5- Varier un paramètre à la fois en fixant les 2 autres
...
%Illustration de la plaque 32X32 avec les positions du récepteur marqué 
...
%Décrire la graphique où on voit la géométrie avec l'emplacement du capteur et
%des sources
...
\begin{figure}[H]
  \centering
  \includegraphics[scale=0.25]{resultats2.png}
